...
export default async function handler(req, res) {
  ...
  try {
    ...
    // MARKET REGIME FILTER — if SPY hasn't moved 3% in 10 days, market is choppy — skip
    ...
    if (marketIsChoppy) {
      return res.status(200).json({
        ...
        spy10Range: spy10Range.toFixed(1),
        market_regime: 'CHOPPY',
        picks: [],
        market_summary: `Market is in consolidation — SPY has only moved ${spy10Range.toFixed(1)}% over the last 10 days. This strategy works best in trending markets. Waiting for a clear directional move before surfacing signals.`,
        marketNote: 'No trades today — choppy market conditions. Check back when SPY makes a clear directional move.',
