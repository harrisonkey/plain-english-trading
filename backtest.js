...
export default async function handler(req, res) {
  ...
  try {
    // Fetch SPY directly - skip pre-test
    console.log('Fetching SPY data...');
    const spyUrl = `https://api.polygon.io/v2/aggs/ticker/SPY/range/1/day/${fromStr}/${toStr}?adjusted=true&sort=asc&limit=500&apiKey=${POLY}`;
    console.log('SPY URL:', spyUrl.replace(POLY, 'KEY_HIDDEN'));
    const spyR = await fetch(spyUrl);
    ...
    for (let b = 0; b < TICKERS.length; b += BATCH_SIZE) {
      ...
      for (const f of batchResults) {
        ...
        for (let i = 22; i < bars.length - 40; i += 3) {
          ...
          // Market regime filter — skip if SPY range over last 10 days < 3%
          const spySlice = spyBars.filter(b => new Date(b.t) <= new Date(bar.t)).slice(-10);
          if (spySlice.length >= 5) {
